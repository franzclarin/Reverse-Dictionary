...
- **`@vercel/analytics`** — Vercel Web Analytics, mounted as `<Analytics />` in `app/layout.tsx` (merged 2026-08-31; the dashboard integration opened two identical PRs, #1 merged and #2 closed as a duplicate). Client-side page-view beacons only; it touches neither `/api/lookup` nor the retrieval path.
  - **It does not log query text, and that is a property of the code, not a setting.** `useRoute()` in `@vercel/analytics/next` builds the beacon from `usePathname()`, which excludes the query string, so `/search?q=<the description>` is reported as `path: "/search"`. `searchParams` is read *only* to derive a route pattern via `computeRoute()`, never to build the reported path. The component also passes `route`, which sets `disableAutoTrack`, so the remote `/_vercel/insights/script.js` never reports a URL of its own. **Analytics still sends no query text anywhere, and that is unaffected by RD-24** — re-verify these two mechanisms before upgrading the package. (RD-24 started logging query text *to our own database* on 2026-08-31; it did not send any to Vercel, and the "Established facts" bullet now records both halves.)
  - **What it does record is the answer.** A word page reports `path: "/word/petrichor"` with `route: "/word/[word]"`. So analytics can tell you which words users land on, but never what they typed to get there.
...
- **Original model artifacts: the HF repo, NOT the zip (corrected RD-22, 2026-08-30).** `reverse_dict_model.zip` was recorded here as living at the repo root and holding `model.safetensors`; **it is not on disk, and neither is `_model_tmp/`**. The recoverable source is the Hub: `franzclarin/ReverseDictionary` ships `model.safetensors` alongside the full sentence-transformers layout (`1_Pooling/config.json`, `modules.json`, `config_sentence_transformers.json`, `sentence_bert_config.json`), so `SentenceTransformer("franzclarin/ReverseDictionary")` loads it as a **trainable PyTorch model** with the correct `Transformer → Pooling → Normalize` stack. `npm run fetch-model` deliberately fetches only the 4 files the ONNX serving path needs, which is why the safetensors copy is not in `models/`. A gitignored file named in the docs is a claim nobody re-checks — same failure shape as the stale 512 MB ceiling RD-17 corrected.
...
Models: `Word`, `VocabEmbedding`, `GlossEmbedding`, `ShadowLookup`, `QueryLog`.
- `QueryLog { id, searchId @unique, query, k, results Json, resultCount, timingMs, createdAt }` — one row per non-debug `/api/lookup` **search** (RD-24, 2026-08-31), holding the description as typed and the exact `[{word, similarity}, …]` array returned, in rank order. `results` is `jsonb` so the ranking survives and the words cannot drift out of step with their scores. Indexed on `createdAt`. **This is the table that retired "no query text has ever been logged"** — see "Established facts" for what that does and does not license.
  - **`searchId` is per *search*, not per request, and the unique index is a correctness guarantee rather than housekeeping (RD-25, 2026-09-01).** RD-24 shipped writing a row per *request*, and the UI sent two — see "Conventions & gotchas". The client now sends one id per search, so a search that arrives twice (a retry, a double-fired effect) conflicts and is skipped instead of becoming a second row. Asking for more results is a **different** search and gets its own id, so "Load more" still logs. A caller that sends no id — curl, a script, the eval harness — gets a server-generated one and so is logged every time it asks.
...
- `lookup/` — embedding search (primary). No auth, no rate limiting. Writes a `QueryLog` row per non-debug **search** (RD-24, corrected RD-25): **awaited**, so the log is complete rather than whatever survived the response, and wrapped in `try/catch`, so failing to record a search can never fail the search. The insert is `createMany({ skipDuplicates: true })` — `ON CONFLICT DO NOTHING` on `searchId` — which states that a duplicate arrival is expected and fine rather than sending a routine event down the error path. `searchId` is client-supplied, so it is **validated** like `query` and `k`. `/explain`'s `debug: true` requests are excluded, the same way the shadow log excludes them. `QUERY_LOG_ENABLED` in the route turns it off without touching anything else.
- `word/[word]/` — word page data.

## Conventions & gotchas

- **An effect that causes something must clean up after itself, and StrictMode is what finds the ones that don't (RD-25, 2026-09-01).** `next.config.js` sets `reactStrictMode: true`, so in **development** React mounts → unmounts → remounts every component and runs each effect twice — deliberately, to surface effects that are unsafe to repeat. `components/SearchResults.tsx` POSTed to `/api/lookup` from an effect with **no cleanup**, so every search was sent twice. Its `requestIdRef` guard looks like it covers this and does not: it suppresses stale *state updates*, while the discarded request still travels and still costs a full embed + pgvector query. **The waste predates RD-24 by months** — adding a log is what made it visible, as two rows per search. The fix is an `AbortController` aborted from the effect's cleanup, plus `if (err.name === "AbortError") return;` in the catch, or a cancelled request renders as a user-facing error. **Before assuming a duplicate write is a server bug, check whether the client sent it twice**: `createdAt − timingMs` gives the request's start time, and two rows starting 1ms apart is a client firing twice, not a server writing twice.
- **Resetting state on a prop change belongs in a `key`, not a second effect (RD-25).** `SearchResults` synced `k` back to 10 with `useEffect(() => setK(INITIAL_K), [query])` beside the effect that fetches on `[query, k]`. After a "Load more" a new query fired the fetch with the **stale** `k`, then again with the reset one — two requests, different `k`, in **every** environment including production. `app/search/page.tsx` now passes `key={query}`, so a new query is a new component instance with fresh state and there is no cross-effect ordering to get right. This deleted code rather than adding it.
...
- **The WordNet training corpus is unsplit and unusable for evaluation.** The model card (`README.md` in the HF repo; originally read from `reverse_dict_model.zip`, which is gone — RD-22) records 181,149 (gloss, lemma, negative-lemma) triplets, `MultipleNegativesRankingLoss`, **3 epochs, no evaluator, no held-out split** (`eval_on_start: False`, `prediction_loss_only: True`). Its sample rows are verbatim WordNet glosses. Any WordNet-gloss-derived eval slice is ~100% leaked. This is why the eval set is hand-authored.
...
- **No query text was logged before 2026-08-31 — RD-24 retired that property deliberately.** For the whole history that produced every number on this page, nothing stored a query: before RD-06 removed them, `Lookup` was `{id, userId, createdAt}` — a rate-limit counter — and `GameRound` was the credits casino (`coinflip`, `slots`, …), not a word game; neither ever stored query text, and `ShadowLookup` stores a sha256 and top-1 only, by design. **That is why the eval set is hand-authored, and nothing about the frozen set changes** — `v1.jsonl` stays frozen and every recorded run predates the first logged query. As of RD-24, `/api/lookup` writes a `QueryLog` row per non-debug search holding the description as typed plus every word and score returned. Two things that must travel with any use of it: those rows are **unlabelled** — they carry no correct answer, so they can measure phrasing, length and popularity but **never accuracy** (this is exactly `lib/shadowLookup.ts`'s "agreement is not accuracy" point) — and at this app's traffic there is not yet enough of them to build a set from, which is why **RD-10's Stack Exchange harvest is still the right move**, not something this replaces.
...
- **`probe-margins.ts`'s +0.134/+0.094 figures describe the LEMMA index only.** They are Phase A3: 25 hand-picked queries, top 10, `VocabEmbedding`. For the gloss index the equivalent is RD-21's `probe-rank1-margin.ts` — whole frozen set, serving path, depth 100 — which measures **+0.070**. Do not quote the older pair as current.
...
- **The headroom every ticket cites is not near-misses (RD-21, 2026-08-28).** "77% of targets are inside the top 100 but only 24% at rank 1" is exact and has been repeated across four tickets, where it is read as *a reranker is 53 points away*. Measured on the shipped index at depth 100 (`npm run probe:margin`): of the **58.5%** in the shortlist but not first, **near-ties (gap < 0.03) are only 26.2% — 73.8% is a confident margin**. Median gap **0.061**; mean winner similarity **0.736** against a target's **0.667**. The gloss cutover roughly *halved* the lemma index's margin (+0.094 → +0.070) and did not close it, so **a third of the headroom is still a gap wider than the entire margin RD-02 existed to fix**. This is a second, independent reason RD-12's reranker failed: the job was never "break ties". **Caveat that must travel with the number:** the widest gap in the set is `merriness` losing to `cheerfulness`, where the model is arguably right and `acceptable[]` is incomplete — only 133/312 rows carry one, so **73.8% is an upper bound on the model's fault**, and classifying it is RD-21's step 1. The margin is a property of the *bi-encoder's* score surface; a cross-encoder rescores from scratch and does not inherit it.
...
| **`prod_wikt_shipped`** (693,325 rows, lists=833, probes=100) | **25.1%** | 20.6% | **55.7%** | 17.4% | **what ships** — RD-17 |
...
> **Echo in the `prod_wikt_shipped` row was corrected 17.5% → 17.4% on 2026-08-30 (RD-22).** Recomputed from the committed run on the authored-reachable slice it is **17.44%**, and the per-row `echo` field and an independent recomputation of the `probes.ts` rule agree exactly. 17.5% appears to have been transcribed from `rd17_prod_p200` (17.47%), which shares every other digit in that table. No verdict depends on it; recorded rather than silently fixed, because two conventions under one column name is the kind of thing that gets compared across tables later.
...
## Training (`training/`) — Python, dev-only (RD-22)

**There was no training code in this repo until 2026-08-30.** The original fine-tune ran in Colab, outside version control. `training/` is the missing half: a `uv`-managed Python 3.12 environment, a shared library (`training/rdlib/`) and five notebooks that fine-tune either model, score the result, and hand it back to the TypeScript harness. **Nothing here is imported by the app and nothing is installed on Vercel** — the serving path has no Python in it.

```bash
brew install uv && cd training && uv sync
uv run python -m ipykernel install --user --name reverse-dictionary
export EVAL_CELL_DIR=~/rd_eval_cells    # the darwin default is under os.tmpdir() and GETS REAPED
```

- **`rdlib` is a second implementation of scoring and retrieval, and it is pinned.** METHODS is right that two scorers can drift; the repo's answer everywhere else is a check (`verify-viz` pins the browser tokenizer to the real `AutoTokenizer`; `rd17_control` had to reproduce `full_gloss_ft` to the digit), so `rdlib.parity.report()` re-derives the published figures, the paired McNemar tests and the echo rate from the committed runs, and compares the PyTorch encoder against the served ONNX. **It raises on failure — run it after touching anything in `rdlib/`.**
- **Measured at build time: 9/9 checks pass, and a Python-built cell scored by BOTH scorers agrees 287/287** on deep rank, top-1 and full top-10 order. That cell also reproduced RD-16's `full_gloss_ft` control exactly (25.4 / 21.6 / 51.6 / 0.304 / 14.6%).
...
- **RD-24 is done** — `/api/lookup` now writes a `QueryLog` row per non-debug search (query as typed, results, similarities). It retires the "no query text has ever been logged" property; the rows are **unlabelled**, so they characterise phrasing and never accuracy, and **RD-10 is not superseded by it**.
...
# RD-22: the Python training surface (see "Training" above). Dev-only.
cd training && uv sync                       # once; needs `brew install uv`
export EVAL_CELL_DIR=~/rd_eval_cells         # the darwin default GETS REAPED
uv run python -c "from rdlib import parity; parity.report()"   # THE GATE — run after any rdlib change
uv run jupyter lab                           # then start with notebooks/00_setup_and_parity.ipynb
...
npm run probe:margin         # RD-21: how far below rank 1 the target actually sits
