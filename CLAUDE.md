...
- **`@vercel/analytics`** — Vercel Web Analytics, mounted as `<Analytics />` in `app/layout.tsx` (merged 2026-08-31; the dashboard integration opened two identical PRs, #1 merged and #2 closed as a duplicate). Client-side page-view beacons only; it touches neither `/api/lookup` nor the retrieval path.
  - **It does not log query text, and that is a property of the code, not a setting.** `useRoute()` in `@vercel/analytics/next` builds the beacon from `usePathname()`, which excludes the query string, so `/search?q=<the description>` is reported as `path: "/search"`. `searchParams` is read *only* to derive a route pattern via `computeRoute()`, never to build the reported path. The component also passes `route`, which sets `disableAutoTrack`, so the remote `/_vercel/insights/script.js` never reports a URL of its own. **"No query text has ever been logged" under "Established facts" still holds** — re-verify these two mechanisms before upgrading the package, because that fact is why the eval set is hand-authored (RD-10).
