...
- `QueryLog { id, query, k, results Json, resultCount, timingMs, createdAt }` — one row per non-debug `/api/lookup` request (RD-24, 2026-08-31), holding the description as typed and the exact `[{word, similarity}, …]` array returned, in rank order. `results` is `jsonb` so the ranking survives and the words cannot drift out of step with their scores. Indexed on `createdAt`. **This is the table that retired "no query text has ever been logged"** — see "Established facts" for what that does and does not license.
...
- `lookup/` — embedding search (primary). No auth, no rate limiting. Writes a `QueryLog` row per non-debug request (RD-24): **awaited**, so the log is complete rather than whatever survived the response, and wrapped in `try/catch`, so failing to record a search can never fail the search. `/explain`'s `debug: true` requests are excluded, the same way the shadow log excludes them. `QUERY_LOG_ENABLED` in the route turns it off without touching anything else.
